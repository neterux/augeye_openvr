...
#include <mutex>
//#include <thread>
//#include "EyeDetector/tracker.h"
#include "EyeDetector/EyeCam.h"
...
public:
    ...
    void AddPlaneToScene( Matrix4 mat, std::vector<float>& vertdata );
    ...
    void UpdateScene( vr::Hmd_Eye nEye );
...
private: // OpenGL bookkeeping
    ...
    // added by ikeda
	GLuint m_iTexture[2];
    std::vector<unsigned char> imageRGBA[2];
    unsigned nImageWidth, nImageHeight;
    unsigned nTextureWidth, nTextureHeight;
...
bool CMainApplication::BInit()
{
    ...
    // Outer camera init
    m_camera[vr::Eye_Left].Init(vr::Eye_Left + 1);
    m_camera[vr::Eye_Right].Init(vr::Eye_Right + 1);
		...
		{
            {
                ...
                m_key = sdlEvent.key.keysym.sym;
            }

			if ( m_key == SDLK_ESCAPE || m_key == SDLK_q )
			{
				bRet = true;
			}
			if ( m_key == SDLK_c )
...
void CMainApplication::RunMainLoop()
{
    ...
    //std::thread t_eyetrack(EyeTrack);
	...
	while ( !bQuit )
	{
        ...
        //std::cout << m_udp.udp_recv() << std::endl;
	}
...
bool CMainApplication::SetupTexturemaps()
{
	...
	//std::string strFullPath = Path_MakeAbsolute( "../left.png", sExecutableDirectory );
    //lodepng::decode( imageRGBA[vr::Eye_Left], nImageWidth, nImageHeight, strFullPath.c_str() );

    nImageWidth = m_camera[vr::Eye_Left].GetWidth();
    nImageHeight = m_camera[vr::Eye_Left].GetHeight();
    ...
    //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, nTextureWidth, nTextureHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, &imageRGBA[vr::Eye_Left][0]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, nTextureWidth, nTextureHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, m_camera[vr::Eye_Left].GetImagePtr());
    ...
    //right
    //strFullPath = Path_MakeAbsolute("../right.png", sExecutableDirectory);
    //lodepng::decode(imageRGBA[vr::Eye_Right], nTextureWidth, nTextureHeight, strFullPath.c_str());

    glBindTexture(GL_TEXTURE_2D, m_iTexture[vr::Eye_Right]);

    //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, nTextureWidth, nTextureHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, &imageRGBA[vr::Eye_Right][0]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, nTextureWidth, nTextureHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, m_camera[vr::Eye_Right].GetImagePtr());
...
void CMainApplication::SetupScene()
{
	...
	glBufferData( GL_ARRAY_BUFFER, sizeof(float) * vertdataarray.size(), nullptr, GL_DYNAMIC_DRAW );
...
void CMainApplication::RenderStereoTargets()
{
    ...
    //glBindTexture( GL_TEXTURE_2D, m_iTexture[vr::Eye_Left]);
    //static int visit = 0;
    //glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, nImageWidth, nImageHeight, GL_RGBA, GL_UNSIGNED_BYTE, !visit ? &imageRGBA[vr::Eye_Left][0] : &imageRGBA[vr::Eye_Right][0]);
    //visit = (++visit) % 2;
...
void CMainApplication::RenderScene( vr::Hmd_Eye nEye )
{
	...
	if( m_bShowCubes )
	{
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, nImageWidth, nImageHeight, GL_RGBA, GL_UNSIGNED_BYTE, m_camera[nEye].GetImagePtr());
        glBindVertexArray( m_unSceneVAO );
...
//{
//    bool SUCCESS = true;
//    Tracker m_eyetrack[2];
//
//    // EyeTracker init
//    if (m_eyetrack[vr::Eye_Left].Init("Pupil Cam1 ID1") != SUCCESS)
//        return;
//    //if (m_eyetrack[vr::Eye_Right].Init("Pupil Cam1 ID0") != SUCCESS)
//    //    return;
//
//    isTracking = true;
//    while (isTracking)
...
//            switch (m_key) {
//            case 'r':
//                m_eyetrack[vr::Eye_Left].eye_model_updaters->reset();
//                //m_eyetrack[vr::Eye_Right].eye_model_updaters->reset();
//                break;
//            case 'p':
//                m_eyetrack[vr::Eye_Left].eye_model_updaters->add_fitter_max_count(10);
//                //m_eyetrack[vr::Eye_Right].eye_model_updaters->add_fitter_max_count(10);
//                break;
//            case 'k':
//                isTracking = false;
//                break;
...
//        }
//
//        int nEye = vr::Eye_Left;
//        {
//            cv::Mat& img = m_eyetrack[nEye].eyecam.Capture();
//
//            // Undistort a captured tmpimg
//            //m_eyetrack[nEye].camera_undistorters->undistort(img, img);
//
...
//            bool is_pupil_found = m_eyetrack[nEye].pupilFitter.pupilAreaFitRR(img_grey, rr_pf, inlier_pts);
//
...
//            //if (is_pupil_found) {
//            //    if (m_eyetrack[nEye].eye_model_updaters->is_model_built()) {
//            //        ellipse_reliability = m_eyetrack[nEye].eye_model_updaters->compute_reliability(img, el, inlier_pts);
//            //        is_reliable = (ellipse_reliability > REL_TREATH);
//            //    }
//            //    else {
//            //        is_added = m_eyetrack[nEye].eye_model_updaters->add_observation(img_grey, el, inlier_pts, FORCE_ADD);
//            //    }
//            //}
//
//            // Visualize results
...
//            //    singleeyefitter::EyeModelFitter::Circle curr_circle = m_eyetrack[nEye].eye_model_updaters->unproject(img, el, inlier_pts);
//            //    m_eyetrack[nEye].ppl = curr_circle.centre.data();
//            //    m_eyetrack[nEye].eybl = m_eyetrack[nEye].eye_model_updaters->fitter().eye.centre.data();
//
//            //    if (is_reliable) {
//            //        m_eyetrack[nEye].eye_model_updaters->render(img_rgb, curr_circle);
//            //    }
//            //}
//            //else {
//            //    m_eyetrack[nEye].eye_model_updaters->render_status(img_rgb);
//            //}
//
//            cv::imshow(m_eyetrack[nEye].window_names, img_rgb);
//        }
//        //
//        //nEye = vr::Eye_Right;
//        //{
...
//
//    // EyeTracker init
//    if (m_eyecam[vr::Eye_Left].Open("Pupil Cam1 ID1") != SUCCESS)
//        return;
//    if (m_eyecam[vr::Eye_Right].Open("Pupil Cam1 ID0") != SUCCESS)
//        return;
//
//    isTracking = true;
//    while (isTracking)
//    {
//        cv::waitKey(1);
//        cv::imshow("Left", m_eyecam[vr::Eye_Left].Capture());
//        cv::imshow("Right", m_eyecam[vr::Eye_Right].Capture());
//
//        {
//            std::lock_guard<std::mutex> lock(m_mtx);
//            if (m_key == 'r')
...
//    bool SUCCESS = true;
//    EyeCam m_eyecam[2];
//    Detector2D detector[2];
//    Detector2DProperties prop =
//    {
//        23,     // int   intensity_range;
//        5,      // int   blur_size;
//        160,    // float canny_treshold;
//        2,      // float canny_ration;
//        5,      // int   canny_aperture;
//        100,    // int   pupil_size_max;
//        10,     // int   pupil_size_min;
//        0.6,    // float strong_perimeter_ratio_range_min;
//        1.1,    // float strong_perimeter_ratio_range_max;
//        0.8,    // float strong_area_ratio_range_min;
//        1.1,    // float strong_area_ratio_range_max;
//        5,      // int   contour_size_min;
//        0.09,   // float ellipse_roundness_ratio;
//        4.3,    // float initial_ellipse_fit_treshhold;
//        0.5,    // float final_perimeter_ratio_range_min;
//        1.0,    // float final_perimeter_ratio_range_max;
//        3.0,    // float ellipse_true_support_min_dist;
//        2.0     // float support_pixel_ratio_exponent;
//    };
//    cv::Mat img[2];
//    std::shared_ptr<Detector2DResult> result;
//    std::vector<std::string> window_name = { "Left", "Right" };
//
//    // EyeTracker init
//    if (m_eyecam[vr::Eye_Left].Open("Pupil Cam1 ID1") != SUCCESS)
//        return;
...
//        {
//            img[nEye] = m_eyecam[nEye].Capture();
//        }
//
//        for (int nEye = 0; nEye < 2; nEye++)
//        {
//            cv::Mat img_gray;
//            cvtColor(img[nEye], img_gray, CV_RGB2GRAY);
//
//            cv::Rect roi = { 0, 0, m_eyecam[nEye].width, m_eyecam[nEye].height };
//
//            result = detector[vr::Eye_Left].detect(prop, img_gray, img[nEye], img_gray, roi, false, false);
//
//            cv::Point center(result->ellipse.center.x(), result->ellipse.center.y());
//            cv::Size axes(result->ellipse.major_radius, result->ellipse.minor_radius);
//            cv::ellipse(img[nEye], center, axes, result->ellipse.angle, 0, 360, cv::Scalar(0, 255, 0));
//
//            cv::imshow(window_name[nEye], img[nEye]);
//        }
//
//        cv::waitKey(1);        
//    }
