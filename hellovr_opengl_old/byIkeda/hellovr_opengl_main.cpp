...
bool CMainApplication::BInit()
{
    ...
    // EyeTracker init
    //m_eyetrack[vr::Eye_Left].Init("Pupil Cam1 ID1");
    //m_eyetrack[vr::Eye_Right].Init("Pupil Cam1 ID0");

    //m_eyecam[vr::Eye_Left].Open("Pupil Cam1 ID1");
    //m_eyecam[vr::Eye_Right].Open("Pupil Cam1 ID0");
...
void CMainApplication::RunMainLoop()
{
	...
	while ( !bQuit )
	{
        ...
        //EyeTrack(vr::Eye_Left);
        //EyeTrack(vr::Eye_Right);
	}

	SDL_StopTextInput();
}

void CMainApplication::EyeTrack(vr::Hmd_Eye nEye)
{
    char keyInput = cv::waitKey(1);
    if (nEye == vr::Eye_Left)
    {
        cv::imshow("Left", m_eyecam[nEye].Capture());
    }
    else
    {
        cv::imshow("Right", m_eyecam[nEye].Capture());
    }
...
//    switch (keyInput) {
//    case 'r':
//        m_eyetrack[nEye].eye_model_updaters->reset();
//        break;
//    case 'p':
//        m_eyetrack[nEye].eye_model_updaters->add_fitter_max_count(10);
//        break;
//    default:
//        break;
//    }
...
void CMainApplication::RenderStereoTargets()
{
    ...
    //glBindTexture( GL_TEXTURE_2D, m_iTexture[vr::Eye_Left]);
    //glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, nImageWidth, nImageHeight, GL_RGBA, GL_UNSIGNED_BYTE, m_camera[vr::Eye_Left].GetImagePtr());
    //static int visit = 0;
	...
	glBindFramebuffer( GL_FRAMEBUFFER, rightEyeDesc.m_nRenderFramebufferId );
 	glViewport(0, 0, m_nRenderWidth, m_nRenderHeight );
    //glBindTexture( GL_TEXTURE_2D, m_iTexture[vr::Eye_Right] );
    //glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, nImageWidth, nImageHeight, GL_RGBA, GL_UNSIGNED_BYTE, m_camera[vr::Eye_Right].GetImagePtr());
    //glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, nImageWidth, nImageHeight, GL_RGBA, GL_UNSIGNED_BYTE, !visit ? &imageRGBA[vr::Eye_Left][0] : &imageRGBA[vr::Eye_Right][0]);
